...
from services.drs_close_service import process_drs_close
...
# Класс UI для окна выборочной печати на закрытие (Дорремстрой)
class GuiDrsCloseEx(QDialog):
    ...
    def __init__(self, parent=None, order_data: dict = None):
        ...
        self.setWindowTitle("Выборочная печать — Дорремстрой")
        self.resize(800, 550)
    ...
    def init_ui(self):
        ...
        # 2. Список документов
        docs_data = [
            ("drs_prper", "Акт приёма-передачи", "1"),
            ("f7", "Форма 7", "1"),
            ("rozpiska", "Расписка", "1")
        ]
    ...
    # Метод подключение сервиса печати (с сообщение после печати)
    def on_print_clicked(self):
        ...
        process_drs_close(self.order_data, selected_docs=selected_docs)
        cleanup_temp_files()
