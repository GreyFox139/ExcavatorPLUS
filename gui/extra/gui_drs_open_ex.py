...
# Класс UI окна выборочной печати на открытие (Дорремстрой)
class GuiDrsOpenEx(QDialog):
        ...
        for doc_id, controls in self.doc_inputs.items():
            ...
            if cb.isChecked():
                try:
                    ...
                    copies = 1
                selected_docs[doc_id] = copies

        if not selected_docs:
            return

        process_drs_open(self.order_data, selected_docs=selected_docs)
        cleanup_temp_files()
