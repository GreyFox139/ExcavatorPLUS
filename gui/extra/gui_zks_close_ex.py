...
from services.zks_close_service import process_zks_close
...
# Класс UI окна выьорочной печати на закрытие (Жилкомсервис)
class GuiZksCloseEx(QDialog):
    ...
    def __init__(self, parent=None, order_data: dict = None):
        ...
        self.setWindowTitle("Выборочная печать — Жилкомсервис")
        self.resize(800, 620)
    ...
    def init_ui(self):
        ...
        # 2. Список документов
        docs_data = [
            ("zks_prper", "Акт приёма-передачи", "1"),
            ("zks_act", "Акт комиссии", "1"),
            ("f7", "Форма 7", "1"),
            ("rozpiska", "Расписка", "1")
        ]
        ...
        for doc_id, controls in self.doc_inputs.items():
            ...
            if cb.isChecked():
                ...
                if not copies_raw.isdigit():
                    show_warning_message(self, "Скольок-сколько копий?\nШифратор, что ли?")
                    inp.setFocus()
                    return False
    ...
    def on_print_clicked(self):
        ...
        process_zks_close(self.order_data, selected_docs=selected_docs)
        cleanup_temp_files()
